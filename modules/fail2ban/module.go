...
func (m *Module) Description() string { return "Intrusion prevention software framework that protects computer servers from brute-force attacks" }
...
func (m *Module) Status(ctx context.Context) (*contract.RuntimeStatus, error) {
	...
	_, stdout, _, _ := m.agent.CommandExecute(ctx, "fail2ban-client", "-v")
	// fail2ban-client -v output is usually: Fail2Ban v0.11.2
	...
	return &contract.RuntimeStatus{
		...
		SoftwareVersion: strings.TrimSpace(stdout),
	}, nil
}
...
func (m *Module) Actions() []contract.ActionDef {
	return []contract.ActionDef{
		{ID: "reload", Title: "Reload", Icon: "refresh-cw"},
		{ID: "restart", Title: "Restart", Icon: "rotate-cw", Dangerous: true, RequiresConfirmation: true},
	}
}
