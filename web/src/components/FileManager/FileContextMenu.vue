<template>
  <div v-if="menu.show"
    ...
    <button v-if="!menu.file?.is_dir" @click="$emit('copy', menu.file)" class="w-full text-left px-4 py-2 hover:bg-slate-700 hover:text-white transition-colors flex items-center">{{ $t('fileManager.copy') }}</button>
    <button @click="$emit('move', menu.file)" class="w-full text-left px-4 py-2 hover:bg-slate-700 hover:text-white transition-colors flex items-center">{{ $t('fileManager.move') }}</button>
    <button v-if="!menu.file?.is_dir && archive" @click="$emit('extract', menu.file)" class="w-full text-left px-4 py-2 hover:bg-slate-700 hover:text-white transition-colors flex items-center">{{ $t('fileManager.extract') }}</button>
